#!/bin/bash
# Link verifier/rust against the Z3 SDK under ~/lib/ (not pysmt-install's 4.8.x).
...
if [[ ! -f "$Z3_LIB_DIR/libz3.so" && ! -f "$Z3_LIB_DIR/libz3.a" ]]; then
    ...
    echo "run: python3 verifier/download_z3.py z3-4.16.0 --sdk $Z3_PREFIX --bindir ~/bin" >&2
