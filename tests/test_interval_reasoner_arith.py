...
def test_rewrite_intervals_removes_mod_when_no_overflow():
    p = int(ARGS().field_type.value)
    x = Symbol("x", INT)
    assumptions = [LE(Int(0), x), LE(x, Int(100))]

    out = rewrite_intervals([Mod(x, Int(p))], assumptions=assumptions)[0]
    assert not _has_mod(out)
    assert out == x


def test_rewrite_intervals_keeps_mod_when_negative_possible():
    p = int(ARGS().field_type.value)
    x = Symbol("x", INT)
    y = Symbol("y", INT)
    assumptions = [
    ...
    out = rewrite_intervals([Mod(x - y, Int(p))], assumptions=assumptions)[0]
    assert _has_mod(out)
...
def test_mod_zero_to_eq_zero_in_injective_window():
    ...
    out = rewrite_intervals([f], assumptions=assumptions)[0]
