...
def test_divides_inside_or_and_not():
    # The hypothesis shape: equalities buried in an Or must still be normalized.
    # Field content-division makes every modular atom monic in its linear content:
    # 6x+12 -> x+2, and 6x+13 -> x-14 (since x ≡ -13·6⁻¹ ≡ 14 (mod 97)).
    x = Symbol("x", INT)
    f2 = Equals(Mod(Plus(Times(Int(6), x), Int(13)), Int(P)), Int(0))
    f2_norm = Equals(Mod(Plus(x, Int(-14)), Int(P)), Int(0))
    scaled = Equals(Mod(Plus(Times(Int(6), x), Int(12)), Int(P)), Int(0))
    divided = Equals(Mod(Plus(x, Int(2)), Int(P)), Int(0))
    out = _asserts(simplify_normalize_eqs(_script(Not(Or(scaled, f2)))))
    assert out == [Not(Or(divided, f2_norm))]
