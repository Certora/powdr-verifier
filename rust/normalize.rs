...
use smt2::{
    assert_commands, contains_bound_var_dyn, declare_fun_name_cmd, has_bool_sort_leaf_dyn,
...
fn collect_bool_symbols_from_ast(ast: &Dynamic, out: &mut HashSet<String>) {
    for n in smt2::iter_nodes_dyn(ast) {
        if n.get_sort().kind() != z3::SortKind::Bool {
            continue;
        }
        if let Some(name) = smt2::symbol_name_dyn(&n) {
            if matches!(n.kind(), AstKind::Var | AstKind::App if n.is_const()) {
                out.insert(name);
            }
        }
    }
...
fn contains_bool_symbol(ast: &Dynamic, _bool_symbols: &HashSet<String>) -> bool {
    has_bool_sort_leaf_dyn(ast)
}
...
pub fn apply(script: &Script) -> Result<(Script, serde_json::Value), String> {
    ...
    let out = map_asserts(script, |b| {
        if has_bool_sort_leaf_dyn(&Dynamic::from_ast(b)) {
            return Ok(b.clone());
        }
...
fn collect_variables(
    ...
    keys.retain(|k| !bool_symbols.contains(k));
    keys.sort();
...
fn expr_to_poly(n: &Int, var_index: &HashMap<String, usize>, modulo: Option<i128>) -> Option<Poly> {
    ...
    match head.as_str() {
        ...
        _ => {
            let key = term_key(n);
            if n.get_sort().kind() == z3::SortKind::Bool || var_index.get(&key).is_none() {
                return None;
            }
