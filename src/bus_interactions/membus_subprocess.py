"""Subprocess client for ``membus.py extract/solve --json`` (match-var preanalysis)."""
...
@lru_cache(maxsize=8)
def _fetch_solve_json_cached(path_str: str, mtime_ns: int) -> dict | None:
    del mtime_ns
    path = Path(path_str)
    try:
        proc = subprocess.run(
            [_MEMBUS_SCRIPT, "solve", "--json", "--file-a", str(path)],
            capture_output=True,
            text=True,
            timeout=30,
            check=False,
        ...
        _LOG.warning("membus solve subprocess failed for %s: %s", path, e)
        return None
    if proc.returncode != 0:
        _LOG.warning(
            "membus solve exited %d for %s: %s",
            proc.returncode,
            path,
    ...
    try:
        ...
        _LOG.warning("membus solve JSON parse failed for %s: %s", path, e)
        return None


def fetch_solve_json(dump_path: Path | None) -> dict | None:
    """Run ``membus solve --json`` on ``dump_path``; ``None`` if unavailable."""
    if dump_path is None or not dump_path.is_file():
        return None
    try:
        mtime_ns = dump_path.stat().st_mtime_ns
    except OSError as e:
        _LOG.warning("membus solve cannot stat %s: %s", dump_path, e)
        return None
    return _fetch_solve_json_cached(str(dump_path.resolve()), mtime_ns)
